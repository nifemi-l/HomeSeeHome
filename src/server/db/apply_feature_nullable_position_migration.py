"""
Apply incremental Feature.x_pos/y_pos/z_pos nullable schema (no wipe).

Run from repo root with PYTHONPATH pointing at server, or from src/server:

    cd src/server
    python3 db/apply_feature_nullable_position_migration.py
...
def main() -> None:
    ...
    sql_path = os.path.join(here, "migrate_feature_nullable_position.sql")
    ...
    try:
        with conn.cursor() as cur:
            for i, stmt in enumerate(statements, start=1):
                ...
                cur.execute(stmt + ";")
        conn.commit()
        print("Feature nullable position migration applied successfully (committed).")
